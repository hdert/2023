...
pub fn main() !void {
    ...
    try stdout.print("Type any amount of numbers you want the result of, finishing with a blank line\n", .{});
    var total = getNumber(buffer[0..]) catch return;
    while (true) {
        const number = getNumber(buffer[0..]) catch |err| switch (err) {
            CalculatorError.EmptyInput => break,
            else => return err,
        };
        total = evaluate(total, number, operator) catch |err| switch (err) {
